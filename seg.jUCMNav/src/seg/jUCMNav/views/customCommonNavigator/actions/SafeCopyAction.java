...
/**
 * The platform's internal navigator CopyAction, extended to stamp every
 * clipboard it writes with {@link JUCMResourceCopyMarkerTransfer}. The marker
 * lets {@link SafePasteAction} decide that the resource payload is trustworthy
 * before it hands it to {@link ResourceTransfer}; see
 * {@link SafePasteAction} for the rationale.
 *
 * @author skuly
 */
/*package*/class SafeCopyAction extends SelectionListenerAction {

	/**
	 * The id of this action.
	 */
	public static final String ID = "seg.jUCMNav.views.customCommonNavigator.actions.SafeCopyAction"; //$NON-NLS-1$
	 ...
	 */
	public SafeCopyAction(Shell shell, Clipboard clipboard) {
		super("Copy");
	 ...
	 */
	public SafeCopyAction(Shell shell, Clipboard clipboard, SafePasteAction pasteAction) {
	 ...
	 * Unlike the platform, every clipboard written here is also stamped with
	 * {@link JUCMResourceCopyMarkerTransfer}.
	 *
		...
		try {
			...
			if (fileNames.length > 0) {
				clipboard.setContents(
						new Object[] { resources, fileNames, names, Boolean.TRUE },
						new Transfer[] { ResourceTransfer.getInstance(),
								FileTransfer.getInstance(),
								TextTransfer.getInstance(),
								JUCMResourceCopyMarkerTransfer.INSTANCE });
			} else {
				clipboard.setContents(
						new Object[] { resources, names, Boolean.TRUE },
						new Transfer[] { ResourceTransfer.getInstance(),
								TextTransfer.getInstance(),
								JUCMResourceCopyMarkerTransfer.INSTANCE });
			}
			...
			if (MessageDialog
					.openQuestion(
							shell,
							"Problem with copy title", //$NON-NLS-1$
